...
func New(name, addr string) *Cluster {
	c := &Cluster{self: Node{name, addr, true}, nodes: map[string]Node{}, ring: hash.New(160)}
	c.nodes[addr] = c.self
	c.ring.Add(addr)
	return c
}
...
func (c *Cluster) StartHeartbeat(ctx context.Context) { go c.Heartbeat(ctx, time.Second) }
