...
type Metrics struct {
	mu                                              sync.Mutex
	requests, hits, misses, latencyNs, maxLatencyNs uint64
}
...
func (m *Metrics) hitRate(r, h, miss uint64) (float64, float64) {
	if r == 0 {
		return 0, 0
	}
	return float64(h) * 100 / float64(r), float64(miss) * 100 / float64(r)
}
func (m *Metrics) HitRate() float64 {
	r, h, _ := m.Snapshot()
	hitRate, _ := m.hitRate(r, h, 0)
	return hitRate
}
func (m *Metrics) MissRate() float64 {
	r, _, miss := m.Snapshot()
	_, missRate := m.hitRate(r, 0, miss)
	return missRate
}
...
func (m *Metrics) Details() Snapshot {
	r, h, miss := m.Snapshot()
	hitRate, missRate := m.hitRate(r, h, miss)
	...
	return Snapshot{r, h, miss, hitRate, missRate, total, max, avg}
}
